...
- v11 解析器优先用本地 PDFium 按文本坐标重建表格行，避免按 PDF 对象写入顺序错配
  标签和金额；必须通过同样的主体、报告期、字段及财务方程校验。保留物理页码、单位
  和文本哈希，空白页不压缩编号。未通过时继续尝试现有文本与 OCR 路径。
- 现有文本路径按 PDF 明确声明的 Adobe 字体集合恢复缺失的 Unicode 映射，仅在内存中操作。
...
- OCR v3 在内存中渲染完整物理页，包含全部图块、文本、遮罩和旋转，避免只取最大嵌入图
  时漏掉标题、单位和标签。使用独立的 `rapidocr-v3-page-N.txt` 缓存，保留 v1/v2 缓存。
  继续使用红色通道减弱印章干扰，原始文件不修改；线程数固定为 1，避免多模型
  争抢本地计算资源。可疑的前导零分组金额不作为有效事实接受。
- 现金流“使用”净额行仅缺左括号时，必须有同页、同口径、同单位的唯一流入和流出小计，
  两者差额为负且绝对值精确匹配原文净额，才允许恢复。保留三行联合源哈希和
  `cash-subtotals-v1` 派生元数据，并继续通过完整现金流方程；不以方程残差猜测金额。
- 正文中的“请参阅财务报表附注”不是报表结束标题；银行四列结构依据真实财务数值行，
  不将日期数字计为数值列，避免母行报表混入合并报表。
- 识别带报告期前缀的合并表标题及“公司报表”边界，避免混合合并口径与母公司口径。
  OCR 交错的收入表头必须同时匹配当期与比较期，不用收入子项替代营业总收入。
- 摘要金额插在换行标签中间时，只有完整标签与相邻数字行对应才采纳；期间标题旁的
  完整人民币单位声明不改变报告期校验。股份变动表的精确期末股数优先于取整股本金额。
- 支持紧邻合并报表标题上方的精确报告期，比较年度不激活本期报表。
  原生坐标中极近的数字片段只在补齐不完整千分组时拼接，不合并独立数值列。
  拆分附注编号不得充当营业成本；经营现金流“（使用）产生”按原披露正负号读取。
  前 20 页明确标注本报告期末日期的公司总股本，使用精确股数并保留来源页码，
  不使用无日期的分红基数或其他年度股数覆盖本期值。
  第 100 页之后的股份变动表须同时明确本报告年度、报告期末、变动前后及股数单位，
  才可采用精确期末股数；不从晚页普通附注行扩大推断。
- 空白银行/保险模板行及被投资保险公司的名称，不是发行人金融机构身份的依据；
  不以此减少非金融企业必须披露的财务字段。
